...
def main(cfg : DictConfig) -> None:
    ...
    for iteration in pbar:
        ...
        if "extend_config" in cfg and len(cfg.extend_config) > 0:
    ...
    if cfg.get("vis", False):
        if "extend_config" in cfg and len(cfg.extend_config) > 0:
            ...
            j3d = fk_return.global_translation[0, :, robot_joint_pick_idx, :] .detach().numpy()
        j3d = j3d - j3d[:, 0:1]
    ...
    joblib.dump((shape_new.detach(), scale), f"data/{cfg.humanoid_type}/shape_optimized_v1.pkl") # V2 has hip joints
